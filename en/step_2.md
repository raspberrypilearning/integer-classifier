...
To train a model — the process of having a model learn rules for classifying things from a series of examples — you'll need a lot of example data for it. Since you want the model to identify handwritten numbers from zero to nine, you'll need lots of images of those numbers written by different people, that are properly labelled so that the model can check its own accuracy as it trains.
